    ...
    <div class="col">
        <section class="card">
            <header class="card-header">
                <a href="{{ route('admin.content.services.create') }}" class="btn btn-primary btn-sm">{{ __('Add') }} <i class="fas fa-plus"></i></a>
            </header>
            <div class="card-body">
            ...
            <div class="card-body">
                <div class="modal-wrapper">
                    ...
                    <div class="modal-text">
                        <p class="mb-0">{{ __('Are you sure that you want to delete this services item?') }}</p>
                    </div>
                </div>
            </div>
...
@section('page_scripts')
    ...
    <script>
        ...
        */
        ...
        @if(session()->has('services_added'))
            new PNotify({
                title: '{{ __('Services') }}',
                text: '{{ session('services_added') }}',
                type: 'success',
                hide: true,
                buttons: {
                    sticker: false
...
@section('livewire_scripts')
    ...
    <script>
        ...
        window.addEventListener('updatetable', event => {
            $('#datatable-services').DataTable().draw();
        })
    </script>
@endsection
