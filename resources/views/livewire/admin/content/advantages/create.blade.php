        ...
        <form id="form" action="" class="form-horizontal">
            <section class="card">
                <header class="card-header">
                    <h2 class="card-title">{{ __('Advantages add') }}</h2>
                </header>
                <div class="card-body">
                    @foreach($advantages_data as $index => $item)
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <div class="input-group">
                                    ...
                                    <input type="text" wire:model.lazy="advantages_data.{{ $index }}.title" class="form-control @error('advantages_data.'. $index .'.title') error @enderror"/>
                                </div>
                                @error('advantages_data.'. $index .'.title')
                            ...
                            <div class="tab-content">
                                @foreach($advantages_data as $index => $item)
                                    <div id="desc_{{ $index }}" class="tab-pane @if($loop->iteration == 1) active @endif">
                                        <textarea class="form-control" wire:model.lazy="advantages_data.{{ $index }}.content"></textarea>
                                    </div>
                                @endforeach

                                @foreach($advantages_data as $index => $item)
                                    @error('advantages_data.'.$index.'.text')
                ...
                <footer class="card-footer">
                    <div class="row justify-content-end">
                        <div class="col-sm-12 text-center">
                            <button class="btn btn-primary" wire:click.prevent="save"><i class='fa fa-spinner fa-spin mr-2' wire:loading wire:target="save"></i>{{ __('Submit') }}</button>
                        </div>
                    </div>
                </footer>
            </section>
