        ...
        <form id="form" action="" class="form-horizontal">
            ...
            <section class="card">
                <div class="card-body">
                    ...
                    @foreach($location_data as $index => $item)
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                ...
                                <span class="input-group-prepend">
                                    ...
                                    <input type="text" wire:model.lazy="location_data.{{ $index }}.country" class="form-control @error('location_data.'. $index .'.country') error @enderror"/>
                                </div>
                                @error('location_data.'. $index .'.country')
                                ...
                                <ul class="nav nav-tabs">
                                    @foreach($location_data as $index => $item)
                                        <li class="nav-item @if($tab_address_active == $index) active @endif">
                                            <a class="nav-link" wire:click.prevent="$set('tab_address_active', {{ $index }})" href="#address_{{ $index }}" data-toggle="tab"><img class="icon-16" src="{{ asset($item['icon']) }}" alt=""></a>
                                        </li>
                                    @endforeach
                                </ul>
                                <div class="tab-content">
                                    @foreach($location_data as $index => $item)
                                        <div id="address_{{ $index }}" class="tab-pane @if($tab_address_active == $index) active @endif">
                                            <textarea class="form-control" wire:model.lazy="location_data.{{ $index }}.address"></textarea>
                                        </div>
                                    @endforeach

                                    @foreach($location_data as $index => $item)

                                        @error('location_data.'.$index.'.address')
                                ...
                                <ul class="nav nav-tabs">
                                    @foreach($location_data as $index => $item)
                                        <li class="nav-item @if($tab_contacts_active == $index) active @endif">
                                            <a class="nav-link" wire:click.prevent="$set('tab_contacts_active', {{ $index }})" href="#desc_{{ $index }}" data-toggle="tab"><img class="icon-16" src="{{ asset($item['icon']) }}" alt=""></a>
                                        </li>
                                    @endforeach
                                </ul>
                                <div class="tab-content">
                                    @foreach($location_data as $index => $item)
                                        <div id="desc_{{ $index }}" class="tab-pane @if($tab_contacts_active == $index) active @endif">
                                            <textarea class="form-control" wire:model.lazy="location_data.{{ $index }}.contacts"></textarea>
                                        </div>
                                    @endforeach

                                    @foreach($location_data as $index => $item)

                                        @error('location_data.'.$index.'.contacts')
                                        <label class="error text-color-danger" for="">
                                            <img class="icon-16" src="{{ asset($item['icon']) }}" alt=""> {{ $message }}
                                        </label>
                                        <br>
                                ...
                                <ul class="nav nav-tabs">
                                    @foreach($location_data as $index => $item)
                                        <li class="nav-item @if($tab_hours_active == $index) active @endif">
                                            <a class="nav-link" wire:click.prevent="$set('tab_hours_active', {{ $index }})" href="#hours_{{ $index }}" data-toggle="tab"><img class="icon-16" src="{{ asset($item['icon']) }}" alt=""></a>
                                        </li>
                                    @endforeach
                                </ul>
                                <div class="tab-content">
                                    @foreach($location_data as $index => $item)
                                        <div id="hours_{{ $index }}" class="tab-pane @if($tab_hours_active == $index) active @endif">
                                            <textarea class="form-control" wire:model.lazy="location_data.{{ $index }}.hours"></textarea>
                                        </div>
                                    @endforeach

                                    @foreach($location_data as $index => $item)

                                        @error('location_data.'.$index.'.hours')
                                        <label class="error text-color-danger" for="">
                                            <img class="icon-16" src="{{ asset($item['icon']) }}" alt=""> {{ $message }}
                                        </label>
                                        <br>
                                ...
                                <div class="isotope-item document col-sm-12 col-md-12 col-lg-12">
                                    <div class="thumbnail">
                                        <div class="thumb-preview">
                                            ...
                                            @else
                                                <a class="thumb-image" href="{{ asset('img/560x370.jpg') }}">
                                                    <img src="{{ asset('img/560x370.jpg') }}" class="img-fluid" alt="">
                                                </a>
                                            @endif
