    ...
    <div class="col">
        <section class="card">
            <header class="card-header">
                <a href="{{ route('admin.pages.page.create') }}" class="btn btn-primary btn-sm">{{ __('Add') }} <i class="fas fa-plus"></i></a>
            </header>
            <div class="card-body">
                <table class="table table-bordered table-striped mb-0" id="datatable-pages">
                    <thead>
                    <th>ID</th>
                    <th>{{ __('Title') }}</th>
                    <th>{{ __('Created') }}</th>
            ...
            <div class="card-body">
                <div class="modal-wrapper">
                    ...
                    <div class="modal-text">
                        <p class="mb-0">{{ __('Are you sure that you want to delete this page?') }}</p>
                    </div>
                </div>
            </div>
...
@section('livewire_scripts')
    ...
    <script>
        ...
        window.addEventListener('updatetable', event => {
            $('#datatable-pages').DataTable().draw();
        })
    </script>
@endsection
